...
public class Jz implements Instruction {
    ...
    public int exec(String eip, int opcode) {
        if(((EFlag) CPU_State.eflag).getZF()){
            ...
            char[] chars= MMU.getMMU().read(logicAddr,8+8);
            int immediate=Integer.parseInt(new Transformer().binaryToInt(String.valueOf(chars).substring(8,16)));
            /**
             * +8
             */
            int a=Integer.parseInt(new Transformer().binaryToInt(eip))+immediate+8;
            CPU_State.eip.write(new Transformer().intToBinary(Integer.parseInt(new Transformer().binaryToInt(eip))+immediate+8+""));
            return 0;
        }else{
